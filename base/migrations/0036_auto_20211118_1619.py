# Generated by Django 3.2.9 on 2021-11-18 16:19
...
def migrate_power_properties(apps, schema_editor):
    ...
    PowerProperty.objects.all().delete()
    pps = []
    for power in Power.objects.all():
        ...
        if power.accessory_type == 'WEAPON':
            pps.append(
                PowerProperty(
                    ...
                    level=power.level,
                    subclass=power.subclass,
                    description=f'Урон $wpn*{power.dice_number}+{power.attack_attribute[:3].lower()}+dmg. {power.hit_effect or ""}',
                )
            )
        if power.accessory_type == 'IMPLEMENT':
            pps.append(
                PowerProperty(
                    ...
                    level=power.level,
                    subclass=power.subclass,
                    description=f'Урон {power.dice_number}{power.get_damage_dice_display()}+${power.attack_attribute[:3].lower()}+dmg. {power.hit_effect or ""}',
                )
            )
        if power.target:
            pps.append(
        ...
        for title, attr in property_title_to_power_attr.items():
            if (description := getattr(power, attr)) :
                pps.append(
                    PowerProperty(
                        power=power,
                        title=title,
