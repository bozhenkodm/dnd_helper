# Generated by Django 3.2.9 on 2021-11-18 10:27
        ...
        if power.accessory_type == 'WEAPON':
            pps.append(
                PowerProperty(
                    ...
                    level=power.level,
                    subclass=power.subclass,
                    description=f'$wpn*{power.dice_number}+{power.attack_attribute[:3].lower()}+dmg. {power.hit_effect or ""}',
                )
            )
        if power.accessory_type == 'IMPLEMENT':
            pps.append(
                PowerProperty(
                    ...
                    level=power.level,
                    subclass=power.subclass,
                    description=f'{power.dice_number}{power.get_damage_dice_display()}+${power.attack_attribute[:3].lower()}+dmg. {power.hit_effect or ""}',
                )
            )
        if power.target:
            pps.append(
